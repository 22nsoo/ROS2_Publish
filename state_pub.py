...
[GPU 사용 안내]
- --device {auto,cpu,cuda} 로 디바이스 선택 (기본: auto → 가능하면 CUDA)
- 기본 FP16(half) 사용. 끄려면 --no-half
"""
...
def main():
    ...
    parser.add_argument('--device', type=str, default='auto', choices=['auto', 'cpu', 'cuda'],
                        help="추론 디바이스 선택 (auto/cpu/cuda)")
    # half를 기본 True로 두고, --no-half로 끌 수 있게 구성
    parser.add_argument('--half', dest='half', action='store_true', help='CUDA에서 FP16 사용 (기본: on)')
    parser.add_argument('--no-half', dest='half', action='store_false', help='FP16 비활성화')
